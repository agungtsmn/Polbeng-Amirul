    ...
    <div class="pd-x-20 pd-sm-x-30 pd-t-2 d-flex align-items-center justify-content-between flex-wrap">
      ...
      <div id="modalCreateUser" class="modal fade">
        <div class="modal-dialog modal-dialog-vertical-center" role="document">
          <form action="/manage/user" method="POST" class="modal-content bd-0 tx-14" enctype="multipart/form-data">
            @csrf
            ...
            <div class="modal-body pd-25">
              ...
              <div class="form-group">
                ...
                <input class="form-control" type="file" name="profile_img" placeholder="Masukkan foto profil">
              </div>
            </div>
      ...
      <div class="br-section-wrapper">
            ...
            <tbody>
              @foreach ($data as $dataUser)
                <tr>
                  ...
                  <td>
                    @if ($dataUser->profile_img)
                      <img class="profile-img" src="{{ Storage::url($dataUser->profile_img) }}" alt="">
                    @else
                      <img class="profile-img" src="{{ asset('/img/placeholder.jpg') }}" alt="">
                    @endif
                    {{ $dataUser->name }}
                  </td>
                  <td class="align-middle">{{ $dataUser->username }}</td>
                  <td class="align-middle">{{ $dataUser->phone_number }}</td>
        ...
        @foreach ($data as $dataUser)
          ...
          <div id="modalUpdateUser{{ $dataUser->id }}" class="modal fade">
            <div class="modal-dialog modal-dialog-vertical-center" role="document">
              <form action="/manage/user/{{ $dataUser->id }}" method="POST" class="modal-content bd-0 tx-14" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="modal-header pd-y-20 pd-x-25">
                  <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">Perbarui Data Pengguna</h6>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
